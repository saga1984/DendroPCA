...
#'@param formato_img formato para las imagenes (jpeg (default), tiff, png, bmp)
#'
...
Dendro_y_PCA <- function(ruta, archivo, nombres_ind, especie, formato_img = jpeg){
  ...
  for(i in 1:length(Best_dendros)){
    ...
    formato_img(paste(ruta, paste(especie, 
                                  "_", 
                                  names(mejores_agrupamientos)[i]), 
                      ".", 
    ...
    formato_img(paste(ruta, paste(especie, "_",names(mejores_agrupamientos)[i]),
                      "_horizontal.", 
                      as.character(substitute(formato_img)), 
                      sep = ""),
                res = 350,
